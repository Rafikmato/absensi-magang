...
class Riwayat extends CI_Controller
{
    ...
    public function detail($id)
    {
        if(isset($_POST['filter']))
        {
            ...
            $query = $this->db->query("SELECT keterangan,id_presensi,jam_masuk,jam_keluar,DATE_FORMAT(tgl_presensi,'%d-%M-%Y') AS tgl FROM presensi WHERE month(tgl_presensi) = '$bulan' AND year(tgl_presensi) = '$tahun' AND keterangan = '$keterangan' AND id_pegawai ='$id'");
            if($query->num_rows() > 0)
            {   
                // echo 'ada';
    ...
    public function riwayat_all()
    {
        ...
        if(isset($_POST['filter']))
        {
            ...
            $tahun = $this->input->post('tahun');
            $bulan = $this->input->post('bulan');
            $query = $this->db->query("SELECT keterangan,id_presensi,jam_masuk,jam_keluar,DATE_FORMAT(tgl_presensi,'%d-%M-%Y') AS tgl FROM presensi WHERE month(tgl_presensi) = '$bulan' AND year(tgl_presensi) = '$tahun' AND keterangan = '$keterangan'");
            if($query->num_rows() > 0)
            {   
                return $this->load->view('dashboard/_absensi_saya',[
                    'data' => $query->result(),
