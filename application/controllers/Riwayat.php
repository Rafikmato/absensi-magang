    ...
    public function detail($id)
    {
        if(isset($_POST['filter']))
        {
            ...
            if($_POST["aksi"] == 'rekap')
            {
                if($bulan == "all")
                {
                    $query = $this->db->query("SELECT keterangan,id_presensi,jam_masuk,jam_keluar,DATE_FORMAT(tgl_presensi,'%d-%M-%Y') AS tgl FROM presensi WHERE year(tgl_presensi) = '$tahun' AND id_pegawai ='$id'");
                    if($query->num_rows() > 0)
                    {   
                        return $this->load->view('dashboard/_detail2',[
                            'data' => $query->result(),
                            'count' => $query,
                            'tahun' => $tahun,
                            'bulan' => $bulan,
